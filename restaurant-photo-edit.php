    ...
    if (isset($_FILES['uploaded_files'])) {
        ...
        foreach ($_FILES['uploaded_files']['name'] as $key => $value) {
            ...
            if (is_uploaded_file($_FILES['uploaded_files']['tmp_name'][$key]) && $_FILES['uploaded_files']['error'][$key] == 0) {
                ...
                $query="insert into restaurant_photo(rest_photo,rest_id)values('{$filename}','{$_SESSION['restaurant_id']}')";
                $result=  mysql_query($query)or die(mysql_error());
                if($result)
                {
                    move_uploaded_file($_FILES['uploaded_files']['tmp_name'][$key], 'admin-panel/rest/rest-photo/' . $filename);
                }
        ...
        header("location:restaurant-display-photo.php?qry={$_SESSION['restaurant_id']}");
            ...
            <section id="content" class="container"> 
                ...
                <section style="width:100%;">
                    <ul class="gallery_fun" style="width:100%;">
                        <?php
                        $query = "select * from restaurant_photo rm where rm.rest_id='{$_SESSION['restaurant_id']}'";
                        $result = mysql_query($query) or die(mysql_error());
                        while ($row = mysql_fetch_array($result)) {
                            echo "<li style='width:285px;margin-bottom:20px;float:left;'>
                            <div class='frame'><a href=''><img src='admin-panel/rest/rest-photo/{$row['rest_photo']}' alt='img' style='height:207px;width:268px;'></a></div>
                          </li>";
                        }
                        ?>
                ...
                <section style="width:100%;">
                    <div style="margin-top:  10px;display: inline-table;">
                        ...
                        <a class="add_field" style="padding: 10px 12px;cursor: pointer;background: #a80000;text-transform:uppercase;color:#fff;text-decoration:none;"> + Add Photo</a> &nbsp;&nbsp;&nbsp;
                        <a class="remove_field" style="padding: 10px 12px;cursor: pointer;background: #a80000;text-transform:uppercase;color:#fff;text-decoration:none;"> - Remove Photo</a>
                        
                        <form action="" method="POST" enctype="multipart/form-data" >
                            <input type="submit" value="Upload Photo" style="padding: 10px 12px;cursor: pointer;background: #a80000;text-transform:uppercase;color:#fff;text-decoration:none;font-size: 14px;margin:-30px 280px;height:40px;vertical-align: middle;">
