...
if (!isset($_SESSION['user_id'])) {
    ...
    if (isset($_GET['qry'])) {
        ...
        $query = "select * from foodie_favorite where foodie_id='{$foodie_id}' and rest_id='{$rest_id}'";
        $result = mysql_query($query) or die(mysql_error());
        $row=  mysql_fetch_array($result);
        if ($row==0) {
           
            $query = "insert into foodie_favorite (foodie_id,rest_id)values('{$foodie_id}','{$rest_id}')";
            $result = mysql_query($query) or die(mysql_error());
            echo "query:<br>$query";
